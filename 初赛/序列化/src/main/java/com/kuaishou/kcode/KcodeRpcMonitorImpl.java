        ...
        for(int i=0;i<utils.size();i++){
            if(temp[i]==0){
                for(int j=i+1;j<utils.size();j++){
                    ...
                    util=utils.get(i);
                    util_2=utils.get(j);
                    if(util.data.equals(util_2.data)){
                        //System.out.println(util_1.data);
                        for(int k=0;k<util.services_len;k++){
                            util.services[k][0]=util.services[k][0]+util_2.services[k][0];
                            util.services[k][1]=util.services[k][1]+util_2.services[k][1];
                        }
                        for(int k=0;k<util.String_IP_len;k++){
                            util.Util_size[k][0]= util.Util_size[k][0]+util_2.Util_size[k][0];
                            util.Util[k] = Arrays.copyOf(util.Util[k], util.Util_size[k][1] + util_2.Util_size[k][1]);
                            System.arraycopy(util_2.Util[k], 0, util.Util[k], util.Util_size[k][1], util_2.Util_size[k][1]);
                            util.Util_size[k][1] = util.Util_size[k][1]+util_2.Util_size[k][1];
                        }
                        prepareResultv1(util);
                        temp[j]=1;
                        break;
                    }
                }
