...
public class Answer {
    ...
    public void addPairData(final String methodAndIpPairString,
                            final int elapsedTime, final boolean success,int flag) {
        PairData pairData=null;
        if(flag==1){
            pairData =this.pairDataMap.computeIfAbsent(methodAndIpPairString, k -> new PairData());
        }
        else{
            pairData=this.pairDataMap.get(methodAndIpPairString);
        }
