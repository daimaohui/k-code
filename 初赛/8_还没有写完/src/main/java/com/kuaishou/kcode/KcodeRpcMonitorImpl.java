...
public class KcodeRpcMonitorImpl implements KcodeRpcMonitor {
    ...
    public static ArrayList<Utils> utils=new ArrayList<>();
    // 不要修改访问级别
                ...
                for(int j=i+1;j<utils.size();j++){
                    ...
                    if(util.data.equals(util_2.data)){
                        for(int k=0;k<util.services_len;k++){
                            util.services[k][0]+=util_2.services[k][0];
                            util.services[k][1]+=util_2.services[k][1];
                        }
                        for(int k=0;k<util.String_IP_len;k++){
                            util.Util_size[k][0]+=util_2.Util_size[k][0];
                            for(int e=0;e<util_2.Util_size[k][1];e++){
                                if (util.Util_size[k][1] == util.Util_size[k][2]) {
                                    util.Util_size[k][2]=util.Util_size[k][2]*5;
                                    util.Util[k] = Arrays.copyOf(util.Util[k], util.Util_size[k][2]);
                                }
                                util.Util[k][util.Util_size[k][1]++]=util_2.Util[k][e];
                            }
