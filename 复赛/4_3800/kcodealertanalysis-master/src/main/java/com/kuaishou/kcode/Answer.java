...
public class Answer {
    ...
    /*用来存放有哪些警告*/
    int warningMaxSize=5000;
    String[][] warning=new String[4][warningMaxSize];
    int warningSize=0;
    /*前向图*/
    ...
    /*记忆化*/
    Map<String,Collection<String>> LongestPathMap=new HashMap<>();
                            ...
                            if(e.ALERT_TYPE.equals("SR")){
                                if(timeIPPair.RS<e.threshold){
                                    if((value+1)>=e.times){
                                        ...
                                        if(warningSize==warningMaxSize){
                                            warningMaxSize=warningMaxSize*2;
                                            warning[0]=Arrays.copyOf(warning[0],warningMaxSize);
                                            warning[1]=Arrays.copyOf(warning[1],warningMaxSize);
                                            warning[2]=Arrays.copyOf(warning[2],warningMaxSize);
                                            warning[3]=Arrays.copyOf(warning[3],warningMaxSize);
                                        }
                                        warning[0][warningSize]=caller;
                                        warning[1][warningSize]=responder;
                                        warning[2][warningSize]=date;
                                        warning[3][warningSize]=e.ALERT_TYPE;
                                        warningSize++;
                                    }
    ...
    public Collection<String> getLongestPathRes(String caller, String responder, String time, String type,String strcom) throws ParseException {
        /*放置路径*/
        timeDataMap.clear();
        rules.clear();
        alarmMonitorrMap.clear();
        alarmMonitorres.clear();
         ...
         * 然后把结果写出来
         */
       LongestPathMap.put(strcom,LongestPath);
        return LongestPath;
    }
    public void getLongestPathRes() throws ParseException {
        /*放置路径*/
//        timeDataMap.clear();
//        rules.clear();
//        alarmMonitorrMap.clear();
//        alarmMonitorres.clear();
         ...
         * 先找好路径
         */
        for(int k=0;k<warningSize;k++){
            String caller=warning[0][k];
            String responder=warning[1][k] ;
            String time=warning[2][k];
            String type=warning[3][k];
            String strcom=caller+responder+time+type;
            if(LongestPathMap.containsKey(strcom)){
                continue;
            }
            Collection<String> LongestPath=new ArrayList<>();
            int startTime = (int) (simpleDateFormat.parse(time).getTime() / (1000 * 60));
