...
public class KcodeAlertAnalysisImpl implements KcodeAlertAnalysis {
    ...
    @Override
    public Collection<String> getLongestPath(String caller, String responder, String time, String type) throws ParseException {
        String strCom=caller+responder+time+type;
        Collection<String> temp=answer.LongestPathMap.get(strCom);
        if(temp!=null){
            return temp;
        }
        else{
            return answer.getLongestPathRes(caller,responder,time,type,strCom);
        }
    }
}