...
public class KcodeAlertAnalysisImpl implements KcodeAlertAnalysis {
    ...
    @Override
    public Collection<String> alarmMonitor(String path, Collection<String> alertRules) throws IOException, InterruptedException {
        ...
        while(mapOffset<fileSize) {
            ...
            while (offset < mapSize) {
                ...
                if(old!=startTime){
                    old=startTime;
                    if(old==mintime+2){
                        Thread getres=new GetRes(answer,mintime);
                        getres.start();
                        prepareRess.add(getres);
                        mintime++;
                    }
                }
