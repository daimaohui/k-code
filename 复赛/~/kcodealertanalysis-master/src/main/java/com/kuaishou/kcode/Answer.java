...
public class Answer {
    ...
    /*前向图*/
    Map<String,ArrayList<Weight>> Node_f=new HashMap<>();
    /*后向图*/
    Map<String,ArrayList<Weight>> Node_s=new HashMap<>();
    ...
    public Collection<String> getLongestPathRes(String caller, String responder, String time, String type) throws ParseException {
        ...
        ArrayList<Weight> weights= Node_f.get(caller);
        String str="";
        System.out.println(caller);
        System.out.println(responder);
        System.out.println(startTime);
        for(Weight weight:weights){
            if(weight.serviceName.equals(responder)){
//                if(weight.SRAndP99Map.containsKey(startTime)) {
//                    if (type.equals("SR")) {
//                        str = weight.SRAndP99Map.get(startTime).SRStr;
//                    } else {
//                        str = weight.SRAndP99Map.get(startTime).P99 + "ms";
//                    }
//
//                }
//                else{
...
//                }
        ...
        System.out.println(path_f.size());
        System.out.println(path_s.size());
    ...
    public void find_f(int startTime,String type,String responder,ArrayList<ArrayList<String>> path,HashSet<String> vis,ArrayList<String> path_1,ArrayList<String> typepath_1,ArrayList<ArrayList<String>> typepath){
        if(Node_f.containsKey(responder)){
            ArrayList<Weight> weights=Node_f.get(responder);
            for(Weight weight:weights){
                if(!vis.contains(weight.serviceName)){
                    path_1.add(weight.serviceName);
                    vis.add(weight.serviceName);
                    if(weight.SRAndP99Map.containsKey(startTime)){
                        if(type.equals("SR")){
                            typepath_1.add(weight.SRAndP99Map.get(startTime).SRStr);
                        }
                        else{
                            typepath_1.add(weight.SRAndP99Map.get(startTime).P99+"ms");
                        }
                    }
                    else{
                        if(type.equals("SR")){
                    ...
                    find_f(startTime,type,weight.serviceName,path,vis,path_1,typepath_1,typepath);
                    typepath_1.remove(typepath_1.size()-1);
                    vis.remove(weight.serviceName);
    ...
    public void find_s(int startTime,String type,String caller,ArrayList<ArrayList<String>> path,HashSet<String> vis,ArrayList<String> path_1,ArrayList<String> typepath_1,ArrayList<ArrayList<String>> typepath){
        if(Node_s.containsKey(caller)){
            ArrayList<Weight> weights=Node_s.get(caller);
            for(Weight weight:weights){
                if(!vis.contains(weight.serviceName)){
                    path_1.add(weight.serviceName);
                    vis.add(weight.serviceName);
                    ...
                    find_s(startTime,type,weight.serviceName,path,vis,path_1,typepath_1,typepath);
                    typepath_1.remove(typepath_1.size()-1);
                    vis.remove(weight.serviceName);
                    path_1.remove(path_1.size()-1);
                }
